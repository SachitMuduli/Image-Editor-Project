...
public class ImageEditorPanel extends JPanel implements KeyListener {
    ...
    public static Color[][] grayscale(Color[][] origJPG) {
        ...
        for (int i = 0; i < origJPG.length; i++) {
            for (int j = 0; j < origJPG[0].length; j++) {
                Color c = origJPG[i][j];
                int newGrayValue = (c.getRed() + c.getGreen() + c.getBlue()) / 3;
    ...
    public static Color[][] rainbowFilter(Color[][] origJPG) {
    ...
    public static Color[][] blurredGrayScale(Color[][] origJPG) { // was going to use this to try and make the canny
                                                                  // edge detector but didn't understand some of the
                                                                  // concepts with it
        final int raidusBlur = 20;
        Color[][] blurredGrayImage = grayscalePhotographicGray(origJPG);
        for (int k = 0; k < origJPG.length; k++) {
            for (int l = 0; l < origJPG[0].length; l++) {
                int totalRed = 0;
                int totalGreen = 0;
                int totalBlue = 0;
                int counter = 0;
                for (int m = k - raidusBlur; m <= k + raidusBlur; m++) {
                    for (int n = l - raidusBlur; n <= l + raidusBlur; n++) {
                        if (m >= 0 && m < origJPG.length && n >= 0 && n < origJPG[0].length) {
                            totalRed += blurredGrayImage[m][n].getRed();
                            totalGreen += blurredGrayImage[m][n].getGreen();
                            totalBlue += blurredGrayImage[m][n].getBlue();
                            counter++;
                        }
                    }
                }
                int redAvg = totalRed / counter;
                int greenAvg = totalGreen / counter;
                int blueAvg = totalBlue / counter;
                Color newBlurredGrayScale = new Color(redAvg, greenAvg, blueAvg);
                blurredGrayImage[k][l] = newBlurredGrayScale;
            }
        }
        return blurredGrayImage;
    }
        ...
        for (int i = 0; i < origJPG.length; i++) {
            for (int j = 0; j < origJPG[0].length; j++) {
                int red = rangeChecker((int) (origJPG[i][j].getRed() * 0.393 + origJPG[i][j].getGreen() * 0.769
                        + origJPG[i][j].getBlue() * 0.189)); // values from stack overflow for the weights for the sepia tone
